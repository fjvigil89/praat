...
def main(path_metadata, label="Sheet1"):    
    ...
    for ind, row in df.iterrows():
        if label=="AVFAD":   ### AAAA sostenida         
            ##nuevas pathology
            #dict_info_signal[row[0]] = {'spk': row[0], 'Path': "data/audio/"+str(label)+"/"+row[0], 'age': row[3], 'gender': row[4], 'tipo': row[15], 'pathology': row[17], 'group': row[18]}
            
            ##old pathology
            dict_info_signal[row[0]] = {'spk': row[0], 'Path': "data/audio/"+str(label)+"/"+row[0], 'age': row[3], 'gender': row[4], 'tipo': row[15], 'pathology': row[15], 'group': row[19]}
        if label=="Saarbruecken":
            ...
            ##nuevas pathology
            dict_info_signal[row[0]] = {'spk': row[0], 'Path': path, 'age': row[5], 'gender': row[4], 'tipo': row[7], 'pathology': str(row[13]).strip().upper(), 'group': row[14]}
